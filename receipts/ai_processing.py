...
def definite_ai_rejection_reasons(receipt: Receipt, result) -> list[str]:
    """AIが明確に不一致と判定した項目だけを再提出候補理由として返す。

    AIは領収書を削除したり再提出依頼を自動作成したりしない。
    読み取り不可・曖昧（None）を含め、最終判断は管理者が領収書本体を確認して行う。
    """
    ...
    if getattr(result, "recipient_name_matches_user", None) is False:
        recipient_name = (getattr(result, "recipient_name", "") or "").strip()
        reasons.append(
            "領収書の利用者名（宛名）が対象ユーザーと一致していません"
            + (f"（読み取り値: {recipient_name}）" if recipient_name else "")
        )
...
def reset_ai_processing_state(receipt: Receipt, *, save: bool = False, clear_extracted_values: bool = False) -> list[str]:
    ...
    receipt.ai_resubmission_recommended = False
...
def apply_ai_filename_to_receipt(receipt: Receipt):
    ...
    # 「再提出不要として確認済み」にするかを人が判断する。
    reasons = definite_ai_rejection_reasons(receipt, result)
    receipt.ai_resubmission_recommended = bool(reasons)
    if reasons:
        receipt.ai_resubmission_recommendation_memo = (
            "AIは再提出候補と判定しました。領収書は削除せず保持しています。"
            "管理者が領収書本体を確認し、再提出依頼を出すか判断してください。理由: "
            + "、".join(reasons)
        )[:2000]
        if receipt.ai_filename_status == ReceiptFilenameStatus.GENERATED:
            ...
            update_fields.append("ai_filename_status")
    else:
        receipt.ai_resubmission_recommendation_memo = ""
    update_fields.extend(["ai_resubmission_recommended", "ai_resubmission_recommendation_memo"])
